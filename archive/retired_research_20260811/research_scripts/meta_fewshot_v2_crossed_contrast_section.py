...
``crossed_contrast_loss``  (X-CON)
    ...
    per-task intercept ``b_t``, and every protein and ligand main effect.  What
    survives is ``G_{P1} - G_{P2}``.  The objective therefore has zero gradient
    through the calibration channel and through the ligand prior, and non-zero
    gradient only where the representation makes the within-task ligand metric
    protein-specific.  Only measured cells are used; no unmeasured pair is
    labelled a non-binder.

Nothing here is authorized for ``model/`` or ``scripts/``.  Precedents that must
be cited when reporting: within-assay relative ranking (MBP), direct
...
class Rectangle:
    ...
    ``support_*``:               support ligand features ``[k, ligand_dim]`` and
                                 ...
                                 rectangle ligands and both of their scaffolds.
    """
...
def combined_objective(model: CrossedContrastSection, episode_terms: list[Tensor],
    ...
    The absolute term is required because the crossed difference identifies no
    offset; the crossed term is required because the absolute term is
    satisfiable by calibration alone.  Neither alone is sufficient and the
    weight is a declared hyperparameter selected on development data only.
    """
    if weight < 0:
