    ...
    for step in range(1, config.steps + 1):
        ...
        if step % config.val_interval == 0 or step == config.steps:
            value = validation_mse(
                ...
                seed=seed + step, y_scale=y_scale)
            if value < best_val:
        ...
        for target in targets:
            for draw in range(draws):
                ...
                if support_mode == "zero":
                    source_y = torch.zeros_like(source_y)
                elif support_mode == "foreign":
                ...
                prediction = model(
                    ...
                    query_baseline_features=baseline_tensors(tensors)[query]).ridge_prediction
...
def run(
    ...
    result = {
        ...
        "g2_authorized": False,
        "g3_authorized": verdict.endswith("PASS"),
