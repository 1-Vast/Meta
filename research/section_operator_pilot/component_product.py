"""F6P component-identifiable biological curve plus task location."""
...
from .gauge_fixed import K


LOCATION_RIDGES = (1.0, 10.0, 100.0)
CV_SEEDS = SEEDS[:5]
ARM_NAMES = (
    "support_free", "location_only", "component_product", "zero_protein",
    "nearest_protein", "wrong_support", "permuted_support",
)
CONTROL_NAMES = (
    "support_free", "location_only", "zero_protein",
    "nearest_protein", "wrong_support",
)


def _location(surface, residual, ridge):
    value = float(np.sum(np.asarray(residual) - np.asarray(surface)))
    return float(np.clip(value / (len(residual) + float(ridge)), -0.5, 0.5))
...
def _evaluate(y, additive, surface, null_surface, protein_views, scaffolds,
    ...
    support_free_interaction_max_error = 0.0
    ...
    for target in range(n_target):
        ...
        for seed_index, seed in enumerate(SEEDS):
            ...
            tau = _location(surface[support, target], residual, location_ridge)
            null_tau = _location(null_surface[support, target], residual, location_ridge)
            nearest_tau = _location(
                surface[support, wrong_target], residual, location_ridge)
            wrong_tau = _location(
                surface[support, target], wrong_residual, location_ridge)
            rng = np.random.default_rng(seed + 65537 * target)
            permuted_tau = _location(
                surface[support, target], residual[rng.permutation(K)], location_ridge)
            permutation_max_error = max(permutation_max_error, abs(tau - permuted_tau))
            pure_location = float(np.sum(residual) / (K + location_ridge))
            curves = {
                "support_free": surface[:, target],
                "location_only": np.full(y.shape[0], pure_location),
                "component_product": surface[:, target] + tau,
                "zero_protein": null_surface[:, target] + null_tau,
                "nearest_protein": surface[:, wrong_target] + nearest_tau,
            ...
            centered_product = curves["component_product"] - float(
            ...
            support_free_interaction_max_error = max(
                support_free_interaction_max_error,
                float(np.max(np.abs(centered_product - centered_support_free))))
    ...
    return (per_target, tau_values, nearest, permutation_max_error,
            support_free_interaction_max_error)


def _summarize(per_target, tau, nearest, permutation_error, interaction_error):
...
def run(args):
    ...
    result = {
        "schema": "MetaSieve.ComponentIdentifiableProduct.F6P.v1",
        ...
        "gate": {"pkis2_raw_components": pkis_raw,
                 ...
                 "verdict": ("F6P_COMPONENTS_ADMISSIBLE" if passed
                             else "F6P_COMPONENTS_NOT_ADMISSIBLE")},
        "read_firewall": {"kcgs_numeric_outcomes": "NOT_READ", "davis_labels": "NOT_READ",
                          "recipient_labels": "NOT_READ"},
        "limitations": [
            "Only task location, not ligand-coupled interaction shape, is adapted from support labels.",
            "The scalar decoder is a feasibility diagnostic; law-valued integration remains separate."],
        "elapsed_seconds": time.time() - started,
    }
    output = Path(args.output).resolve()
    _write_json(output / "result.json", result)
    _write_json(output / "manifest.json", {"sha256": {
        "preregistration": _sha256(Path(__file__).with_name("F6P_PREREGISTRATION.md")),
...
def parser():
    ...
    item.add_argument("--output", default="research/section_operator_pilot/artifacts/f6p")
    return item
