"""Run the lightweight research E-AFF-L0 protein affinity-location Gate.

Everything here is frozen by `EAFF_L0_PREREGISTRATION.md`,
...
STAGE = "P1R2B-E-AFF-L0_PROTEIN_AFFINITY_LOCATION_GATE"
ENDPOINT = "Ki"
MAX_TASKS = 300
LIGANDS_PER_TASK = 20
FEATURE_DIM = 7
...
def run(args) -> dict:
    ...
    if not args.device.startswith("cuda") or not torch.cuda.is_available():
        return fail_closed(output, "L0_NOT_RUN_NUMERICAL_PRECONDITION_FAILED",
    ...
    if ENDPOINT not in estimand["admitted_endpoints"]:
        return fail_closed(output, "L0_NOT_RUN_LOCATION_ESTIMAND_NOT_IDENTIFIED",
    ...
    if contract["verdict"] != "L0_OPERATOR_AND_ANCHOR_CONTRACT_FROZEN":
        return fail_closed(output, "L0_NOT_RUN_OPERATOR_OR_ANCHOR_CONTRACT_FAILED",
                           contract["verdict"])
    ...
    for fold in range(FOLDS):
        ...
        bandwidth = float(train.sum() ** (-1.0 / (FEATURE_DIM + 4.0)))
        for name in arm_names:
            ...
            estimator = KernelConditional(estimator_features[train], y_scaled[train],
                                          grid, bandwidth=bandwidth)
            query = estimator.batch(features[name][test])
    ...
    for name in arm_names:
        ...
        coverage = np.mean((lower <= (grid[None, :] >= y_scaled[:, None]).astype(float))
                           .all(axis=1)
                           & (upper >= (grid[None, :] >= y_scaled[:, None]).astype(float))
                           .all(axis=1))
        metrics[name] = {
    ...
    verdict = ("PROTEIN_SPECIFIC_AFFINITY_LOCATION_IDENTIFIED_IN_SOURCE" if all(passes.values())
               else "PROTEIN_SPECIFIC_AFFINITY_LOCATION_NOT_IDENTIFIED")
...
def parse_args():
    ...
    parser.add_argument("--output", default="research/e0_identifiability/artifacts/eaff_l0_v1")
