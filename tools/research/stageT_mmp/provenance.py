...
isolation, and it is reported that way. `meta_val` and `meta_test` rows are both
...
CACHE = HERE / "T0_PROVENANCE_meta_train.jsonl.gz"

# The corpus endpoint. `exact_labels` also carries Kd rows for the same source
# ids; admitting them would silently mix endpoints.
ENDPOINT = "Ki"
...
def verify_source_authority() -> dict:
    """Both artifacts must hash-match the corpus manifest before anything reads them."""
    ...
    for key, value in expected.items():
        if actual[key] != value:
            raise ValueError(
                f"{key} does not match the corpus manifest: {actual[key]} != {value}")
    return {
        "verified": True,
        "expected": expected,
        "actual": actual,
...
def train_allow_list() -> TrainAllowList:
    """`meta_train` source rows, read from the physically isolated split view."""
    cells: dict[str, dict] = {}
    cell_of_row: dict[str, str] = {}
...
def build_cache(force: bool = False) -> dict:
    """Stream both artifacts once, keep only allow-listed `meta_train` rows."""
    ...
    with gzip.open(EXACT_LABELS, "rt", encoding="utf-8") as handle:
        for line in handle:
            ...
            if row_id not in allow.row_ids:
                # Not bound to anything: the sole use of a non-allow-listed row
                # is this identity test, and it is discarded on the same line.
                continue
...
def load_cache() -> list[dict]:
    if not CACHE.exists():
        raise FileNotFoundError(
            f"{CACHE} is missing; run `python -m "
            f"tools.research.stageT_mmp.t0_reliability` first")
