"""Stage Q frozen evaluation on meta_val (decoupled head; gate: k=0 only)."""
...
from tools.research.stageQ_frozenhead.train_stageq import (
    PANEL_DIM, episode_journal_ids, forward_q, panel_stats,
)
...
def main():
    ...
    model = FrozenHeadModel(
        ...
        journal_vocab=len(vocab), panel_dim=PANEL_DIM,
    ...
    for k, bank in specs.items():
        ...
        for spec in bank:
            ...
            with torch.no_grad():
                ...
                with torch.no_grad():
                    wrong_protein = forward_q(model, wrong_ep, ids, stats, gate)
    ...
    result = {"schema": "MetaSieve.StageQ.Evaluation.v1",
    ...
    print("wrote " + str(args.output) + " and " + str(result_path))
    return 0
