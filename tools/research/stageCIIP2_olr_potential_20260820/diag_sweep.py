...
print('A0 prior R2:', round(O.r2_cells(pc0, tc),4), flush=True)
...
import itertools
for rank, wd in itertools.product([2,8],[1e-4,1e-2]):
    R.RANK = rank
    for mod in ['A5-gain']:
        run = R.train_arm(mod, recs, lig, res_pad, mask_pad, split, seed=11, mhat=mhat, weights=w)
        pc = np.concatenate([run['preds'][j] for j in test_js])
        print('rank', rank, 'wd', wd, 'test R2', round(O.r2_cells(pc, tc),4), 'delta', round(O.r2_cells(pc, tc)-O.r2_cells(pc0, tc),4), flush=True)
R.RANK = 8
