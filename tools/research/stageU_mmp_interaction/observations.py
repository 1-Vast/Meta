...
The primary bank is same-target, exact-positive-uncensored-Ki, same governed
panel. Cross-panel observations are kept separately and are never pooled into
primary statistics.
"""
...
def build_observations(data: QPSMPData, targets: list[str]) -> dict:
    ...
    for index, cell in enumerate(data.cells):
        if cell["split"] != "meta_train" or cell["target_id"] not in target_set:
            continue
            ...
            for position, (left_index, left) in enumerate(entries):
                for right_index, right in entries[position + 1:]:
                    ...
                    observations.append(MMPObservation(
                        ...
                        ligand_a=cell_a["ligand_id"],
                        ligand_b=cell_b["ligand_id"],
                        ...
                        r_a=item.r_a, r_b=item.r_b, context=item.context))
