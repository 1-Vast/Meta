"""Stage E trainer: leak-free baseline T2 vs panel-level orthogonal candidate.

Leak-free protocol copied from Stage B: meta_train components are partitioned
once (seed 20260818) into fit and internal-validation sets; training episodes
come from fit components only; checkpoint selection reads internal-validation
episodes only; meta_val is read exactly once, after freezing.

Arms (single code path, matched budget):
* T2          - incumbent similarity_only trunk + Stage B loss recipe
                (smooth_l1 post + pre + ranknet + centered + dictionary). This
                is the leak-free uniform-training baseline of this stage.
* T2-LEVEL    - loss-only ablation: T2 + the level term routed to the episode
                mean of the zero-shot endpoint.
* LSP         - PanelLevelShapeModel with orthogonal routing: the level head is
                supervised by the level term; the shape path receives the
                centered/ranking terms (the level head's gradient from the
                centered term is identically zero because it is a per-episode
                constant). No smooth_l1 on the full post-adaptation prediction.
* LSP-NOROUTE - framework-only ablation: LSP model with the Stage B loss
                recipe (no routing), so the contact dictionary trains on the
                level-dominated total error as in every failed incumbent.

GPU verification (mandated) runs before the training loop and records:
torch.cuda.is_available(), model parameter devices, batch tensor devices, and
nvidia-smi GPU utilization samples taken during real forward/backward steps.
"""
...
from tools.research.stageB_complementary.train_stageb import (        # noqa: E402
...
ARMS = ("T2", "T2-LEVEL", "LSP", "LSP-NOROUTE")
...
def build_model(config: StageEConfig, data: QPSMPData):
    base = config.base
    if config.arm in ("T2", "T2-LEVEL"):
...
def gpu_probe(data, config: StageEConfig, output_dir: Path) -> dict:
    """Mandated pre-training GPU verification: availability, devices, utilization.

    Runs on a disposable model instance so the probe's own optimization steps
    cannot perturb the training weights.
    """
    ...
    batch_devices = {
        "protein_pooled": str(episode.protein_pooled.device),
        "protein_tokens": str(episode.protein_tokens.device),
        "support_atoms": str(episode.support_atoms.device),
        "query_atoms": str(episode.query_atoms.device),
        "support_y": str(episode.support_y.device),
        "query_y": str(episode.query_y.device),
    }
    model_devices = sorted({str(p.device) for p in model.parameters()})
    utilization = []
    probe = {
        ...
        "batch_device_check": (
            bool(device.startswith("cuda"))
            and "cpu" not in {str(episode.protein_pooled.device),
                              str(episode.query_atoms.device),
    ...
    for step in range(6):
        ...
        loss = F.smooth_l1_loss(
            output.prediction, episode.query_y.to(
                device=output.prediction.device, dtype=output.prediction.dtype))
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()
        if step >= 1:
            utilization.append(sample_util())
    parsed = []
    for sample in utilization:
        if sample and not sample.startswith("unavailable"):
            parts = sample.split(",")
            try:
                parsed.append((int(parts[0].strip()),
                               int(parts[1].strip())))
            except (ValueError, IndexError):
                pass
    probe["nvidia_smi_samples"] = utilization
    probe["forward_backward_steps_executed"] = 6
    probe["max_gpu_utilization_percent"] = max(
        (value for value, _ in parsed), default=None)
    probe["max_memory_used_mb"] = max(
        (value for _, value in parsed), default=None)
    probe["utilization_nonzero"] = any(
        value > 0 for value, _ in parsed)
...
def arm_loss(config: StageEConfig, model, output, episode, query_y, base,
             label_scale):
    """Return (episode_loss, {"loss_level":..., "loss_post":...})."""
    ...
    support_match = output.support_match_loss  # computed on batched occupancy inside forward
    terms = {
    ...
    if config.arm == "T2":
        terms["loss_post"] = 1.0 * F.smooth_l1_loss(output.prediction, query_y)
    elif config.arm == "LSP-NOROUTE":
        terms["loss_post"] = 1.0 * F.smooth_l1_loss(output.prediction, query_y)
    elif config.arm == "LSP":
        # orthogonal routing: level head sees the level term; the shape path
        # gets only the centered/ranking terms (the centered term's gradient
        # into the per-episode level constant is identically zero).
        level_pred = output.additive - output.ligand_only  # panel_level
        target = query_y.mean(-1, keepdim=True).expand_as(level_pred)
        terms["loss_level"] = LEVEL_LOSS_WEIGHT * F.mse_loss(
            level_pred, target.detach())
    elif config.arm == "T2-LEVEL":
        level_pred = output.zero_shot.mean(-1, keepdim=True).expand_as(
            output.zero_shot)
        target = query_y.mean(-1, keepdim=True).expand_as(level_pred)
        terms["loss_level"] = LEVEL_LOSS_WEIGHT * F.mse_loss(
            level_pred, target.detach())
...
def train(data: QPSMPData, config: StageEConfig, output_dir: Path,
    ...
    optimizer = torch.optim.AdamW([
        {"params": slow_parameters, "lr": base.learning_rate * base.backbone_lr_scale},
    ...
    print(f"arm={config.arm} fit components {len(fit_components)}, "
          f"internal components {len(internal_components)}, "
          f"bank episodes {sum(len(b) for b in banks.values())}", flush=True)

    device, dtype = base.device, torch.float32
    best_state, best_value, best_step = None, float("inf"), 0
    trace, term_trace, forwards = [], [], 0
    grad_stats = []
    ...
    for step in range(1, base.steps + 1):
        ...
        if step in (base.steps // 2, base.steps):
            grad_stats.append({
                "step": step,
                "gradient_norms": {
                    ...
                    if param.grad is not None and (
                        name.startswith("panel_level")
                        or name.startswith("contact_weight")
                        or name.startswith("ligand_head")
                        or name.startswith("interaction_head")
                        or name.startswith("protein_encoder")
                        or name.startswith("ligand_encoder"))}})
        if step % base.val_interval == 0 or step == base.steps:
            ...
            progress = {"step": step, "arm": config.arm,
                        ...
                        "gpu_util_pct": util[0] if util else None,
                        "gpu_mem_mb": util[1] if util else None}
    ...
    report = {
        ...
        "loss_trace": trace, "term_trace": term_trace,
        ...
        "gradient_coverage": grad_stats,
        "gpu_probe": probe,
    }
    payload = {"model_state": model.state_dict(),
               "config": asdict(base), "arm": config.arm,
               "label_scale": asdict(label_scale), "stageE_version": 1}
