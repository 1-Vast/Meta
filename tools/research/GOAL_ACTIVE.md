...
current_status: ACTIVE_RESEARCH (performance track open; mechanism successor blocked)
terminal_verdict: CORE_TASK_1_UNRESOLVED; CIIP_1A_ORACLE_LOCAL_NOT_SUPPORTED
authority: task.md
reopening_condition: new preregistration and explicit authorization for any mechanism successor


Goal:
Long-term research and reconstruction of MetaSieve are permitted. The primary
goal is a reproducible, trainable improvement on leak-free cold-target
zero-/few-shot DTA, with MSE <= 1.00 pK^2 as an aspiration and CI/Spearman,
novelty and bootstrap safeguards. A separately attributable training innovation
is required. Core Task 1 is an attribution question about transferable
protein-conditioned interaction, not a prerequisite for all practical
performance experiments. Closed-form adaptation, query-label leakage and
uncontrolled data mixing remain prohibited; production integration requires
validated evidence and explicit authorization.

Current state:
- performance track: independently runnable under a fresh preregistration;
- Core Task 1: UNRESOLVED overall;
- CIIP-1A: ORACLE_LOCAL_SIGNAL_NOT_SUPPORTED for the tested oracle-local method;
- context-only predictive value: NOT_EVALUATED;
- CIIP-1B / BindingDB bridge / production integration: NOT_AUTHORIZED.

The remaining dated round notes below are historical log entries. They do not
override the current state above.
