"""Stage J frozen evaluation on meta_val (read once, after freezing)."""
...
from tools.research.stageJ_assay.train_stagej import (
    episode_journal_ids, forward_j, journal_vocabulary,
)
...
def main():
    ...
    specs = data.fixed_nested_episode_banks(
        "meta_val", SUPPORT_SIZES, 16, 2, 73101, None)
    ...
    for k, bank in specs.items():
        for spec in bank:
            ...
            with torch.no_grad():
                ...
                if k == 0:
                    continue
                frozen = forward_j(model, episode, ids)
                ...
                with torch.no_grad():
                    wrong_protein = forward_j(model, wrong_ep, ids)
    ...
    result = {"schema": "MetaSieve.StageJ.Evaluation.v1",
