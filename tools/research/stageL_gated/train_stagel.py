"""Stage L trainer: support-gated assay-aware level head (k=0-specialized).

Reuses the Stage J model and forward path; the only differences are the
support gate (level head active only at k=0, in training and inference) and
the arm name. Leak-free internal checkpoint selection; GPU verification.
"""
...
from tools.research.stageJ_assay.train_stagej import (
    build_model as build_j, episode_journal_ids, forward_j,
    journal_vocabulary,
)
...
def forward_gated(model, episode, ids, gate):
    """forward_j with the level gate injected."""
    ...
    return model(
        ...
        journal_ids=ids, level_gate=gate, task_state_override=None,
...
def gpu_probe(data, config, model, output_dir):
    ...
    parsed = []
    for step in range(4):
        output = forward_gated(model, episode, ids, 1.0)
...
def train(data, config, output_dir, progress_path):
    ...
    label_scale = training_label_scale(data)
    model, vocab = build_j(config, data, True)
    ...
    print("arm=L vocab " + str(len(vocab)) + " fit " + str(len(fit_components))
    ...
    for step in range(1, base.steps + 1):
        ...
        gate = 1.0 if support_size == 0 else 0.0
        selected = []
    ...
    report = {"arm": "L", "best_internal_val_mse_pk": best_value,
    ...
    payload = {"model_state": model.state_dict(),
               ...
               "arm": "L", "label_scale": asdict(label_scale),
               "stageL_version": 1}
...
def main():
    ...
    train(data, StageEConfig(base=config, arm="L"), args.output,
          progress_path)
    return 0
