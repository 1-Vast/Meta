"""Stage Q2d-1d gate adjudication (frozen gate, never moved).
...
def main():
    ladder = json.loads((HERE / "Q2D1D_LADDER.json").read_text(encoding="utf-8"))
    ...
    gate_pass = out["levels"]["M1:A"]["pass"]
    out["GATE_PASS"] = bool(gate_pass)
    json.dump(out, open(HERE / "Q2D1D_GATE.json", "w"), indent=1)
