...
def build_ligand_48(lig_feats_resolved, train_lig_mask):
    rng = stable_rng("stageQ2d1c", "truth", "wl")
    density = 0.1
...
def generate_truth(mechanism, seed, P_t, L_t, splits):
    rng = stable_rng("stageQ2d1c", "truth", mechanism, "seed", seed)
    n_rows, n_lig = len(P_t), len(L_t)
    d_p = P_t.shape[1]
    d_l = L_t.shape[1]
    if mechanism in ("M1", "M3"):
        A = rng.normal(0, 1, size=(d_p, RANK))
    ...
    I = I_c / sd_tr * TAU
    noise_rng = stable_rng("stageQ2d1c", "truth", mechanism, "noise", seed)
...
def main():
    ...
    rng_split = stable_rng("stageQ2d1c", "splits")
    ...
    np.savez(HERE / "q2d1c_features.npz", P_t=P_t, L_t=L_t, W=W,
             PCA_VT=PCA_VT, resolved_idx=resolved_idx)
    import hashlib
    fsha = hashlib.sha256((HERE / "q2d1c_features.npz").read_bytes()).hexdigest()
    splits_out = {}
    ...
    splits_out["schema"] = "MetaSieve.StageQ2d1c.SPLITS.v1"
    json.dump(splits_out, open(HERE / "Q2D1C_SPLITS.json", "w"), indent=1)
    pre = {"schema": "MetaSieve.StageQ2d1c.ORACLE_PRECHECK.v1",
    ...
    json.dump(pre, open(HERE / "Q2D1C_ORACLE_PRECHECK.json", "w"), indent=1)
