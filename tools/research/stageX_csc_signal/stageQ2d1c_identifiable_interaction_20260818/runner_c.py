"""Stage Q2d-1c ladder runner (features 32-dim protein, 48-dim ligand,
157 resolved ligands). Ladder A-E with value-level reproduction checks,
8 arms, gate on double-cold, NC1/NC2 negative controls, M2/M3 mechanism
sweep at level A.
...
import truth_c as truth

PREREG_SHA = "25b8b9129120d0a770ba353cd56a8a388dd847778e4e5cb2b488ee0cbfee7106"
SEEDS = [0, 1, 2]
RANK = 4
HID = 32
BATCH = 1024
...
def train_level(P, arm, t, level, seed, splits, device, restart, Lt_dev):
    rng_steps = stable_rng("stageQ2d1c", "steps", "seed", seed, "phase", level,
    ...
    if level == "C":
        obs = stable_rng("stageQ2d1c", "missing").random(len(tr)) < 0.70
        tr = tr[obs]
...
def run_seed_arms(P_t, L_t, splits, device, Lt_dev, arm_inputs, mech, level, seed):
    ...
    for arm in arm_inputs:
        ...
        for r_ in range(n_rest):
            model, val, n_cens = train_level(P, arm, t, level, seed, splits, device, r_, Lt_dev)
            n_cens_total += n_cens
...
def main():
    ...
    splits = json.loads(open(HERE / "Q2D1C_SPLITS.json", encoding="utf-8").read())
    ...
    rng_arm = stable_rng("stageQ2d1c", "arms")
    ...
    out = {"schema": "MetaSieve.StageQ2d1c.LADDER.v1",
           "preregistration_sha256": PREREG_SHA,
           "results": results, "censored_counts": cens,
           "repro_A_value_level": repro}
    json.dump(out, open(HERE / "Q2D1C_LADDER.json", "w"), indent=1)
